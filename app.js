...
const mobileNavBtnBars = document.querySelectorAll("nav button > div");
const mobileNav = document.querySelector(".mobile__nav");
const navLinks = mobileNav.querySelectorAll(".mobile__nav a");
...
// Seleccionamos todos los <section>
  ...
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        // Si la sección está en el viewport, le agregamos una clase
        entry.target.classList.add("animate__animated");
        entry.target.classList.add("animate__fadeInUp");
        // Opcional: dejar de observar después de entrar al viewport una vez
        // observer.unobserve(entry.target);
      }
    });
  },
  {
    threshold: 0, // el 10% del section debe estar visible para activar
  }
);
