...
const sectionsToObserve = document.querySelectorAll("section[id], header");
...
function updateActiveLink(id) {
  ...
  mobileNavLinks.forEach(link => link.classList.remove("nav__link--active"));
  
  // Add active class to corresponding links
  const desktopLink = document.querySelector(`.nav__link[href="#${id}"]`);
  const mobileLink = document.querySelector(`.nav__mobile-link[href="#${id}"]`);
  
  if (desktopLink) desktopLink.classList.add("nav__link--active");
  if (mobileLink) mobileLink.classList.add("nav__link--active");
}
...
const observerCallback = (entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      ...
      // Special case for hero section (header)
      else if (entry.target.tagName.toLowerCase() === "header") {
...
document.addEventListener(
  "DOMContentLoaded",
  function () {
    const particleElements = document.querySelectorAll(".particles");
...
document.addEventListener(
  "DOMContentLoaded",
  function () {
    var particleElements = document.querySelectorAll(".particles");
    particleElements.forEach(function (el) {
      particleground(el, {
        dotColor: "#5cbdaa",
        lineColor: "#5cbdaa",
